...
public class Addition<T extends PrimitiveType> implements IValueExpression<T> {
	
	private IValueExpression<T>[] expressions = null;

	public Addition(IValueExpression<T>[] expressions) {
	...
	public T eval(MultiOpTuple tuple) {
		...
		for (int i = 1; i < expressions.length; i++) {
			result = (T)result.add(expressions[i].eval(tuple));
		}
		return result;
	}
