...
* LOG : Logger
value : int
msh : MasterStatisticsHandler
baseId : int
nodeStack : Deque<Node>
numberRunningMachines : int
systemIsRunning : boolean
pathToQueryDefinition : String
logicalOperators : List<LogicalOperator>
logicalStates : List<LogicalState>
sources : List<LogicalOperator>
sink : LogicalOperator
queryToNodesMapping : Map<Integer, Operator>
starTopology : ArrayList<EndPoint>
rct : RuntimeCommunicationTools
bcu : NodeManagerCommunication
eiu : ElasticInfrastructureUtils
manager : ManagerWorker
monitorMaster : MonitorMaster
port : int
RESET_SYSTEM_STABLE_TIME_OP_ID : int
enableRestAPI : boolean
restAPIPort : int
restAPIRegistry : Map<String, RestAPIRegistryEntry>
restAPIServer : Server
OldInfrastructure(int)
isSystemRunning()
getStarTopology()
addSource(Operator)
loadQuery(LogicalSeepQuery)
_loadQuery(LogicalSeepQuery)
makeDataIngestionModeLocalToOp(Operator)
checkReplicaOperator(Operator, int)
configureRouterStatically()
setEiu(ElasticInfrastructureUtils)
setPathToQueryDefinition(String)
getPathToQueryDefinition()
getMonitorMaster()
getOps()
getElements()
getNodePoolSize()
getNumberRunningMachines()
getRCT()
getBCU()
getEiu()
getBaseId()
addNode(Node)
updateContextLocations(Operator)
setDownstreamLocationFromPotentialDownstream(Connectable, Connectable)
setUpstreamLocationFromPotentialUpstream(Connectable, Connectable)
startInfrastructure()
stopWorkers()
localMapPhysicalOperatorsToNodes()
createInitialStarTopology()
addNodeToStarTopology(int, InetAddress)
removeNodeFromStarTopology(int)
getDataFromFile(String)
deployCodeToAllOperators()
deployCodeToOperator(Operator)
broadcastState(StateWrapper)
broadcastState(Operator)
deployQuery()
reDeploy(Node)
failure(int)
sendCode(Node, byte[])
sendCode(Operator, byte[])
remoteOperatorInstantiation(Operator)
broadcastStarTopology()
init(Operator)
initRuntime(Operator)
reMap(InetAddress, InetAddress)
updateU_D(InetAddress, InetAddress, boolean)
start()
stop(int)
getNodeFromPool()
incrementBaseId()
placeNew(Operator, Node)
placeNewParallelReplica(Operator, Operator, Node)
deployConnection(String, Connectable, Connectable, String)
configureSourceRate(int, int)
getOpIdFromIp(InetAddress)
getNumDownstreams(int)
getNumUpstreams(int)
printCurrentInfrastructure()
saveResults()
switchMechanisms()
getOpType(int)
parallelRecovery(String)
saveResultsSWC()
getOperatorById(int)
parseFileForNetflix()
