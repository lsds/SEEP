...
/**
 * The class <code>IProcessingUnitTest</code> contains tests for the class <code>{@link IProcessingUnit}</code>.
 *
 * @generatedBy CodePro at 18/10/13 19:08
 * @author rc3011
 * @version $Revision: 1.0 $
 */
public class IProcessingUnitTest extends TestCase {
	/**
	...
	public static void main(String[] args) {
		if (args.length == 0) {
			...
			junit.textui.TestRunner.run(IProcessingUnitTest.class);
		} else {
			// Run only the named tests
			TestSuite suite = new TestSuite("Selected tests");
			for (int i = 0; i < args.length; i++) {
				TestCase test = new IProcessingUnitTest();
				test.setName(args[i]);
				suite.addTest(test);
			}
