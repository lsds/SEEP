...
public class SubQueryBufferWindowWrapper {
	...
	IWindowBatch start, end;
	
	/* Current window start and end pointer */
	private int w_, _w;
	
	private Deque<IWindowBatch> batches_;
	private Deque<IWindowBatch> _batches;
	
	private Deque<IWindowBatch> next;
		
	private Map<IWindowBatch, Integer> free;
	...
	/**
	 * Hook into the adding routine for the buffer to make sure that 
	 * if an element is added to the buffer then the wrapper updates 
	 * the window state accordingly.
	 * 
