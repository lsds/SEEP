...
public class IncomingDataHandlerWorker implements Runnable{
    ...
    static Logger logger = org.apache.log4j.Logger.getLogger(IncomingDataHandlerWorker.class);
...
//		}
        ...
        try{
            ...
            if(dsa.getUniqueDso() != null){
                ...
                logger.info("-> Unique data adapter in this node: "+dso);
            }
            else{
                dso = dsa.getDataStructureIForOp(originalOpId);
                logger.info("-> Multiple data adapters in this node");
            }
            //Get inputStream of incoming connection
            InputStream is = upstreamSocket.getInputStream();
           ...
           BufferedInputStream bis = new BufferedInputStream(is);
            ...
            while(goOn){
                ...
                for(TuplePayload t_payload : batch){
                    ...
                    if(owner.checkSystemStatus()){
                        ...
                        reg.setSenderOpId(opId); //sender op-id
                        dso.push(reg, opId);
                    }
                    else{
            ...
            logger.error("-> Data connection closing...");
            upstreamSocket.close();
        }
        catch(IOException io){
            logger.error("-> IncDataHandlerWorker. IO Error "+io.getMessage());
            io.printStackTrace();
        }
    }
}
