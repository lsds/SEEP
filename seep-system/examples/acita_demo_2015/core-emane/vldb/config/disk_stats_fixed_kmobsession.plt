set terminal term 
set output sprintf("%s/%s/%sk/%sm/%ss/disk_stats_fixed_kmobsession_%s%s",outputdir,timestr,k,mob,session,timestr,termext)
...
plot sprintf("%s/%s/%sk/%sm/%ss/cpu-util.csv",outputdir,timestr,k,mob,session) using 193:198 notitle w lines linestyle 1, \
	sprintf("%s/%s/%sk/%sm/%ss/cpu-util.csv",outputdir,timestr,k,mob,session) using 193:199 notitle w lines linestyle 2
