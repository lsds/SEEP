...
plot sprintf("%s/%s/1k-tput.data",outputdir,timestr) using 1:2:xtic(sprintf("%.1f",$1)) title "k=1" w lines linestyle 1, \
	sprintf("%s/%s/2k-tput.data",outputdir,timestr) using 1:2:xtic(sprintf("%.1f",$1)) title "k=2" w lines linestyle 2, \
	sprintf("%s/%s/3k-tput.data",outputdir,timestr) using 1:2:xtic(sprintf("%.1f",$1)) title "k=3" w lines linestyle 3, \
	sprintf("%s/%s/5k-tput.data",outputdir,timestr) using 1:2:xtic(sprintf("%.1f",$1)) title "k=5" w lines linestyle 4, \
