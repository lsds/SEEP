...
def main(k,h,w,hostname):
    ...
    try:
        ...
        worker = start_worker(k, h, worker_logfilename, sim_env)
...
def start_worker(k, h, logfilename, sim_env):
    with open(data_dir+'/'+logfilename, 'w') as log:
        args = ['java', '-DuseCoreAddr=true','-DreplicationFactor=%d'%k,'-DchainLength=%d'%h,'-jar', '%s/../lib/%s'%(eg_dir, seep_jar), 'Worker']
