set terminal term 
set output sprintf("%s/%s/rel_latency_vs_cpudelay_stddev%s",outputdir,timestr,termext)

set title "Relative latency vs operator latency for different replication factors (k)"
set xlabel "Operator latency (ms)"
