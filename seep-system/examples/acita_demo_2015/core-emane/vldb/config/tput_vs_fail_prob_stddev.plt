...
plot sprintf("%s/%s/1k-tput.data",outputdir,timestr) using 1:2 title "k=1" w lines linestyle 1, \
	...
	sprintf("%s/%s/3k-tput.data",outputdir,timestr) using ($1+0.002):2:4 notitle linestyle 3 with yerrorb, \
	sprintf("%s/%s/4k-tput.data",outputdir,timestr) using ($1+0.003):2:4 notitle linestyle 4 with yerrorb, \
	sprintf("%s/%s/5k-tput.data",outputdir,timestr) using ($1+0.004):2:4 notitle linestyle 5 with yerrorb
