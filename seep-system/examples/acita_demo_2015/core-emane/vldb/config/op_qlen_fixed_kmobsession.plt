...
set output sprintf("%s/op_qlen_fixed_kmobsession_%s%s",expdir,timestr,termext)
...
op_qlen_files=system(sprintf("find %s -name 'op_*_qlens.txt' | sed 's/.*op_\\([0-9]\\+\\)_qlens.txt/\\1/'", expdir))
