set terminal term 
set output sprintf("%s/%s/rel_tput_vs_cpudelay_stddev%s",outputdir,timestr,termext)

set title "Relative throughput vs operator latency for different replication factors (k)"
