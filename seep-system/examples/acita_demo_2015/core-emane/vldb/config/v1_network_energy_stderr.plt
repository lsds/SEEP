...
#set output plotname
set terminal term 
set output sprintf("%s/%s/network_energy_stderr%s",outputdir,timestr,termext)

set xlabel "Replication factor (k)" font ", 20"
set ylabel "Network energy usage (Bytes)" font ", 20" offset -2
set yrange [0:*]
#set yrange [0:100]
set tics font ", 16"
...
plot sprintf("%s/%s/all-k-energy.data",outputdir,timestr) using 2:4:xticlabels(3) ls 1 
