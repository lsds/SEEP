...
class MeanderWorker1(CoreService):
    ...
    def generateconfig(cls, node, filename, services):
        ...
        if filename is 'shutdown.sh':
            cfg = "#!/bin/sh\n"
            cfg += "# auto-generated by MeanderWorker1 (worker1.py)\n"
            cfg = "touch worker1.shutdown\n"
            return cfg
        else:
            ...
            if filter(lambda service: service._name == "OLSRETX", services):
                cfg += "cp %s/core-emane/vldb/config/olsrd-net-rates.sh net-rates1.sh\n"%(seep_example_dir)
                cfg += "cp %s/core-emane/vldb/config/olsrd-net-topology.sh net-topology1.sh\n"%(seep_example_dir)
            else:
                cfg += "cp %s/core-emane/vldb/config/net-rates.sh net-rates1.sh\n"%(seep_example_dir)
                cfg += "cp %s/core-emane/vldb/config/net-topology1.sh net-topology1.sh\n"%(seep_example_dir)
            #cfg += "cp %s/lib/%s %s\n"%(seep_example_dir, seep_jar, lib_dir)
            cfg += 'echo "Starting MeanderWorker1 on `hostname`(`hostname -i`)"\n'
