...
public class CoreRE {
	...
	public void initializeCommunications(Map<String, Integer> tupleIdxMapper){
		...
		for(Integer i : idimMap.keySet()){
			LOG.info("Upstream: {} with this mode: {}", i, idimMap.get(i));
		}
	...
	public void setRuntime(){
		...
		if(GLOBALS.valueFor("multicoreSupport").equals("true")){
			if(processingUnit.isMultiCoreEnabled()){
				...
				LOG.info("-> Multi core support enabled");
			}
			else{
				// Start the consumer thread.
				dConsumerH.start();
				LOG.info("-> Multi core support not enabled in this node");
			}
		}
		else{
			// Start the consumer thread.
            ...
            public void run() {
                ...
                while(true) {
                    ...
                    try {
                        ...
                        LOG.info("Exception while processing data " + t.getMessage());
                    }
                }
            }
        });
