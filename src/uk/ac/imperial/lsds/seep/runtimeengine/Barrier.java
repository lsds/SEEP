...
public class Barrier implements DataStructureI {
	...
	public Barrier(int initialNumberOfThreads){
		staticBarrier = new CyclicBarrier(initialNumberOfThreads, new Runnable(){
			@Override
			public void run() {
				executeWhenBarrierHit();
			}
		});
	}

	public void executeWhenBarrierHit(){
	...
	public void reconfigureBarrier(int numThreads){
		...
		staticBarrier = new CyclicBarrier(numThreads, new Runnable(){
			@Override
			public void run() {
				executeWhenBarrierHit();
			}
	...
	public void push(DataTuple dt) {
		...
		try{
			...
			//staticBarrier.await();
			//schfan: not sure what it means... remove
			e.printStackTrace();
